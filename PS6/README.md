...
In Problem Set 6, we will compute the maximum theoretical growth rate of E. coli, an organism that is widely used in industry. We will utilize a JSON file, specifically `e_coli_core.json`, which contains the stoichiometric array of E. coli, to extract certain pieces of information to build a list of metabolites, reactions, upper and lower bound array for each reaction, and lastly, a stoichiometric matrix in order to solve the linear programming problem. 
